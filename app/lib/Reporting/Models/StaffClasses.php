...
class StaffClasses extends \Eloquent {
	...
	public function classdetails($class) {
		return \DB::table($this->table)
							...
							->select(\DB::raw('staff_users.forename AS staff_forename, 
								...
								reports_card.management_confirmed_at as report_confirmed_at,
	...
	public function hodclasses() {
		$query = \DB::table($this->table)
							...
							->select(\DB::raw('staff_classes.class AS class, COUNT(DISTINCT student_users.upn) AS student_count, SUM(if(reports_card.management_confirmed_at IS NOT NULL , 1, 0)) AS confirmed, SUM(if(reports_card.staff_completed_at IS NOT NULL , 1, 0)) AS completed, SUM(if(reports_card.management_flagged_at IS NOT NULL , 1, 0)) AS flagged'));
